/*
  ...
  - Made the column `shortName` on table `Party` required. This step will fail if there are existing NULL values in that column.
...
CREATE TABLE "new_OrganMember" (
    ...
    CONSTRAINT "OrganMember_partyId_fkey" FOREIGN KEY ("partyId") REFERENCES "Party" ("id") ON DELETE SET NULL ON UPDATE CASCADE
);
...
INSERT INTO "new_Party" ("createdAt", "id", "name", "shortName", "updatedAt") SELECT "createdAt", "id", "name", "shortName", "updatedAt" FROM "Party";
DROP TABLE "Party";
...
CREATE TABLE "new_Speech" (
    ...
    CONSTRAINT "Speech_partyId_fkey" FOREIGN KEY ("partyId") REFERENCES "Party" ("id") ON DELETE SET NULL ON UPDATE CASCADE
);
